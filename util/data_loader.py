...
transform_train = transforms.Compose(
    [
        ...
        transforms.RandomResizedCrop(size=imagesize, scale=(0.2, 1.0)),
    ...
    if args.in_dataset == "CIFAR-10":
        ...
        if "val" in split:
            ...
            val_loader = torch.utils.data.DataLoader(
                valset, batch_size=config.batch_size, shuffle=True, **kwargs
            )
        ...
        if "train" in split:
            train_loader = torch.utils.data.DataLoader(
                torchvision.datasets.ImageFolder(
                    os.path.join(root, "train"), config.transform_train_largescale
                ),
        ...
        if "val" in split:
            val_loader = torch.utils.data.DataLoader(
                torchvision.datasets.ImageFolder(
                    os.path.join(root, "val"), config.transform_test_largescale
                ),
                batch_size=config.batch_size,
                shuffle=False,
                **kwargs
...
def get_loader_out(
    args, dataset=("tim", "noise"), config_type="default", split=("train", "val")
):
    ...
    if "val" in split:
        ...
        if val_dataset == "dtd":
            transform = (
                ...
                else config.transform_test
            )
            val_ood_loader = torch.utils.data.DataLoader(
                torchvision.datasets.ImageFolder(
                    root="/nfs/scistore19/locatgrp/bdemirel/data/dtd/images/",
                    transform=transform,
                ),
            ...
            val_ood_loader = torch.utils.data.DataLoader(
                torchvision.datasets.ImageFolder(
                    root="/nfs/scistore19/locatgrp/bdemirel/data/places365/",
            ...
            val_ood_loader = torch.utils.data.DataLoader(
                torchvision.datasets.ImageFolder(
                    root="/nfs/scistore19/locatgrp/bdemirel/data/iNaturalist/",
            ...
            val_ood_loader = torch.utils.data.DataLoader(
                torchvision.datasets.ImageFolder(
                    root="/nfs/scistore19/locatgrp/bdemirel/data/SUN/",
                    transform=config.transform_test_largescale,
                ),
                batch_size=batch_size,
                shuffle=True,
            ...
            val_ood_loader = torch.utils.data.DataLoader(
                torchvision.datasets.ImageFolder(
                    root="/nfs/scistore19/locatgrp/bdemirel/data/Places/",
                    transform=config.transform_test_largescale,
                ),
                batch_size=batch_size,
                shuffle=True,
        ...
        elif val_dataset == "ssbhard":
            from pytorch_ood.dataset.img import SSBHard

            transform_test_largescale_ssbhard = transforms.Compose(
                [
            ...
            ssb_hard_dataset = SSBHard(
                root="/nfs/scistore19/locatgrp/bdemirel/data/",
                download=True,
                transform=transform_test_largescale_ssbhard,
            )
            val_ood_loader = DataLoader(
                ssb_hard_dataset, batch_size=batch_size, shuffle=True, **kwargs
            )
        elif val_dataset == "ninco":
            data_root = (
                "/nfs/scistore19/locatgrp/bdemirel/data/ninco/NINCO/NINCO_OOD_classes"
            )
            ninco_dataset = torchvision.datasets.ImageFolder(
                root=data_root, transform=config.transform_test_largescale
            )
            val_ood_loader = DataLoader(
                ninco_dataset, batch_size=batch_size, shuffle=True, **kwargs
            )
        else:
            val_ood_loader = torch.utils.data.DataLoader(
                torchvision.datasets.ImageFolder(
                    "/nfs/scistore19/locatgrp/bdemirel/data/{}".format(val_dataset),
                    transform=transform_test,
                ),
                batch_size=batch_size,
                shuffle=False,
